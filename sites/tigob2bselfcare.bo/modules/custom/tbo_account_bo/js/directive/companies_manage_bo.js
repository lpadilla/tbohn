...
function CompaniesListController($scope, $http) {
	...
	$scope.doIfChecked = function (value, $event) {
		...
		jQuery('#modal').load('/account/manage/message/changeState/' + $event.target.id + '/' + change_name + ruta + '/' + value).dialog('open');
	...
	$scope.deleteCompany = function ($event, document_number, name) {
		...
		var change_name = name.replace(/ /g, "|");
		jQuery('#modal').load('/account/manage/message/deleteCompany/' + document_number + '/' + change_name + ruta).dialog('open');
