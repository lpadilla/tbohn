...
namespace Drupal\tbo_account_bo\Plugin\Config\form;
...
use Drupal\Core\Url;
use Drupal\tbo_account_bo\Form\CreateCompanyForm;
...
/**
 * Manage config a 'CreateCompanyBoFormClass' block.
 */
class CreateCompanyBoFormClass {
  ...
  public function __construct() {
    ...
    $this->service_enterprise = \Drupal::service('tbo_account_bo.create_companies_service');
  }
  ...
  public function buildForm(array &$form, FormStateInterface &$form_state) {
    ...
    $form['enterprise_name'] = [
      ...
      '#value' => $this->_validateService($document, $document, $number),
      '#required' => TRUE,
    ...
    $form['closet'] = [
      '#markup' => '<a href="#" class="modal-close-b2b create-account waves-effect btn btn-second">Cancelar</a>',
  ...
  public function validateForm(array &$form, FormStateInterface &$form_state) {
    ...
    if ($aux_document) {
      ...
      $document = $form_state->getValue('document_type');
      $validation = $this->_validateService($document, $document, $number);
  ...
  public function submitForm(array &$form, FormStateInterface &$form_state) {
    ...
    $repository = \Drupal::service('tbo_account.repository');
    $repository1 = \Drupal::service('tbo_account_bo.repository');
    $segment = 'segmento'; 
    ...
    try {
      ...
      $client_code = $this->_validateService(1, $form_state->getValue('document_type'),$form_state->getValue('document_number'));
      $segmentation = $this->_validateService(2, $form_state->getValue('document_type'),$form_state->getValue('document_number'));
    ...
    try {
      if ($form_state->getValue('user') == 'create') {
        ...
        $phone_number = $this->_validatePhone($phone_number, $prefix_country);
        $document_number = $form_state->getValue('document_number');
        $document_type = $form_state->getValue('document_type');
        ...
        try {
          ...
          $urlForSMS = $GLOBALS['base_url'];

          $sms_message = t('Hola @username, se ha creado una empresa y usted es el administrador, puede iniciar sesion haciendo clic en @urlsite.', array('@username' => $tokens['user'], '@urlsite' => $urlForSMS));
          $sms_message_thx = t('Gracias');
          ...
          $response = $this->api->sendSMS($params);  //llamada a servicio para envio de SMS
          if($response === FALSE){
            \Drupal::logger('Create enterprise')->error("Error enviando SMS de invitación a $phone_number");
          }else{
            \Drupal::logger('Create enterprise')->info("Se envió SMS de invitación a $phone_number");          
          }
        } catch (Exception $e) {
          \Drupal::logger('Create enterprise')->error($e);
        }
      }
      else {
        if ($form_state->getValue('user') == 'associate') {
          ...
          $phone_number_user = $repository1->getUserPhoneByUid($aux_user[0]->uid);
          //send sms
          ...
          try {
            ...
            $sms_message = t('Hola @username, se ha creado una empresa y usted es el administrador, puede iniciar sesion haciendo click en @urlsite.', array('@username' => $aux_user[0]->name, '@urlsite' => $url));          
            $sms_message_thx = t('Gracias');
            $params['query'] = [
              'from' => 'Tigo',
              'to' => $phone_number_user[0]->phone_number,
              'text' => $sms_message . ' ' . $sms_message_thx,            
            ];
            ...
            $number_aa=$phone_number_user[0]->phone_number;
            if($response === FALSE){
              \Drupal::logger('Create enterprise')->error("Error enviando SMS de invitación a $number_aa");
            }else{
              \Drupal::logger('Create enterprise')->info("Se envió SMS de invitación a $number_aa");          
            }
          } catch (Exception $e) {
            \Drupal::logger('Create enterprise')->error($e);
          }
   ...
   * @return string
   */
  public function _validateService($document, $type, $number) {
    ...
    $response = $this->api->getEnterprise($params);

    if($type == 'ci'){
      $name = $response->Envelope->Body->getClientAccountGeneralInfoMobileResponse->client->name;
      $firstLastName = $response->Envelope->Body->getClientAccountGeneralInfoMobileResponse->client->firstLastName;
      $secondLastName = $response->Envelope->Body->getClientAccountGeneralInfoMobileResponse->client->secondLastName;

      $businessName = $name." ".$firstLastName." ".$secondLastName;

    }elseif($type == 'nit'){
      $businessName = $response->Envelope->Body->getClientAccountGeneralInfoMobileResponse->client->businessName;      
    }

    $contracts = $response->Envelope->Body->getClientAccountGeneralInfoMobileResponse->contracts->contract;
    $segmentation = $response->Envelope->Body->getClientAccountGeneralInfoMobileResponse->client->segmentation;
    ...
    if(isset($businessName)){
      if($document == 1 ){
        ...
        if($count > 1){
          ...
          $client_code = implode(",",$temp);
          return array($client_code);
        }else{          
          return $response->Envelope->Body->getClientAccountGeneralInfoMobileResponse->client->clientCode;
        }        
      }
      if($document == 2){
        return $segmentation;
