...
use Drupal\tbo_account_bo\Form\CreateUsersBoForm;
use Drupal\tbo_entities\Entity\CompanyUserRelations;
use Drupal\tbo_entities\Entity\InvitationAccessEntity;
use Drupal\user\Entity\User;

/**
 * Manage config a 'CreateUsersBoFormClass' block.
 */
class CreateUsersBoFormClass {
  ...
  public function createInstance(CreateUsersBoForm &$form) {
    $this->instance = &$form;
  }
  ...
  public function buildForm(array &$form, FormStateInterface &$form_state) {
    ...
    $form['button-wrapper']['closet'] = array(
      '#markup' => '<a href="#" data-ng-click="usersListClear()" class="modal-action modal-close create-account waves-effect btn btn-second">Cancelar</a>',
    );
  ...
  public function validateForm(array &$form, FormStateInterface &$form_state) {
    ...
    $line_number = $this->_validatePhone($line_number, $prefix_country);
  ...
  public function submitForm(array &$form, FormStateInterface &$form_state) {
    ...
    $message = "rol: ".$rol;
    \Drupal::logger('test_rol')->error($message);
    ...
    try {
      if ($success) {
        ...
        try {
          ...
          \Drupal::logger('email')->error("Error enviando correo de invitación a $email");
          
        }
        
        try {
           #se valida si el numero es valido
          ...
          $sms_message = t('Hola @username, se ha creado una cuenta para usted, puede iniciar sesion haciendo clic en @urlsite.', array('@username' => $tokens['user'], '@urlsite' => $urlForSMS));
          ...
          if($response === FALSE){
            \Drupal::logger('Create User')->error("Error enviando SMS de invitación a $line_number");
          }else{
            \Drupal::logger('Create User')->info("Se envió SMS de invitación a $line_number");          
          }
        } catch (Exception $e) {
          \Drupal::logger('Create User')->error($e);
        }
