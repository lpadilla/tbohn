...
namespace Drupal\tbo_account_bo\Repository;
...
/**
 * Class AccountRepositoryBo
 * @package Drupal\tbo_account_bo\Repository
 */
class AccountRepositoryBo extends AccountRepository{
   ...
   * @return mixed
   */
     public function getQueryCompaniesBo($filters = [], $colums_table = [], $config_paginate = [], $others_field = [], $tigoadmin = '') {
    ...
    */
    ...
    /**
     * Desactivar un usuario especifico por medio del email dado.
     *
     * @param $email
     * @return mixed
     */
    public function DesactivarUsuario($email,$nombre)
    {
        if ( (isset($email)) && (isset($nombre)) )
        {
            ...
            if ($user)
            {
                ...
                if ($user->save())
                {
                    ...
                    $respuesta['message'] = 'El Usuario ' . $nombre . ' fue Eliminado';
                }
                else
                {
                    $respuesta['success'] = TRUE;
                    $respuesta['info'] = 'error';
                    $respuesta['message'] = 'No se ha podido Eliminar al Usuario' . $nombre. ', por favor intenta de nuevo';
                }
            }
            else
            {
                $respuesta['success'] = TRUE;
                $respuesta['info'] = 'error';
                $respuesta['message'] = 'El email '.$email.' no corresponde a un usuario valido, por favor intenta de nuevo';
            }
        ...
        else
        {
            $respuesta['success'] = TRUE;
            $respuesta['info'] = 'error';
            $respuesta['message'] = 'Error en los datos ingresados, por favor intenta de nuevo';
        }
        return $respuesta;
    }
   ...
   */
    ...
    public function getUserPhoneByUid($uid = NULL) {
      $query = $this->database->select('users_field_data', 'user');
      $query->fields('user', ['phone_number']);
