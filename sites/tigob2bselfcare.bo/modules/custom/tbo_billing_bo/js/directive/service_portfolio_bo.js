...
function ngCurrentInvoice($http, apiBatch, dataCollector) {
				...
				if (scope.alldata.hasOwnProperty(i)){
					for(var j in scope.alldata[i]){
						if(scope.alldata[i].hasOwnProperty(j)){
							for (var w in scope.alldata[i][j]){
						    if (scope.alldata[i][j].hasOwnProperty(w)){									
									if (scope.alldata[i][j][w]['msisdn']){
										options = [];
										if (!duplicate_autocompletelinea[scope.alldata[i][j][w]['msisdn']]){
											duplicate_autocompletelinea[scope.alldata[i][j][w]['msisdn']] = 'exist';
											options['name'] = scope.alldata[i][j][w]['msisdn'];
											autocompletelinea.push(options);											
										}
									}
									if (scope.alldata[i][j][w]['address']){
										options = [];
										if (!duplicate_autocompleteaddress[scope.alldata[i][j][w]['address']]){
										duplicate_autocompleteaddress[scope.alldata[i][j][w]['address']] = 'exist';
										options['name'] = scope.alldata[i][j]['address'];
										autocompleteaddress.push(options);										
										}
									}
									if (scope.alldata[i][j][w]['contract']){
										options = [];
										if (!duplicate_autocompletecontract[scope.alldata[i][j][w]['contract']]){
											duplicate_autocompletecontract[scope.alldata[i][j][w]['contract']] = 'exist';
											options['name'] = scope.alldata[i][j][w]['contract'];
	  ...
	  scope.checkKeyDownReference2 = function (){	  
		...
		scope.autocompleteChange = function (linea,contrato){			
				...
				if (scope.selectedCategory.length == 0){
					for (var i in scope.alldata){						
						if (scope.alldata.hasOwnProperty(i)){						
							for (var j in scope.alldata[i]){
								if (scope.alldata[i].hasOwnProperty(j)){  								  
									for (var k in scope.alldata[i][j]){
										if (scope.alldata[i][j].hasOwnProperty(k)){
											lengthline=linea.length;
											lengthcontract=contrato.length;		
											if(lengthline > 1 | lengthcontract > 1 ){  											
													...
													if(linea == ''){
														for(var w = 0, lengthcontract; w < lengthcontract; w++){
															...
															if(contrato[w]==[j]){
																val = true;
																length = length + 1;
																aux.splice(contrato, 0, scope.alldata[i][j][k]);
																jstring = [j].toString();
																if(jstring != jstringaux){
																	aux2.push(jstring);
																}	
																aux2.forEach(function(value2 ,key2){
																	arr=[];
																	for (index = 0; index < aux.length; index++) {
																		const element = aux[index];																			
																		if(value2 == aux[index].contract){
																			arr.push(element);
																		}
																		a[value2]=arr;
																	}			
                              ...
                              jstringaux = jstring;
															if([j]==scope.alldata[i][j][k].contract){
																if( linea[z] == scope.alldata[i][j][k].msisdn ){
																	val = true;
																	length = length + 1;
																	aux.splice(linea[z], 0, scope.alldata[i][j][k]);																	
																	jstring = [j].toString();
																	if(jstring != jstringaux){
																		aux2.push(jstring);
																	}	
											...
											}else if (scope.alldata[i][j][k].contract == contrato | scope.alldata[i][j][k].msisdn == linea ){//only one line or one contract										
												if( scope.alldata[i][j][k].msisdn == linea && scope.alldata[i][j][k].contract == contrato){ //caso donde ambos estan seteados												
												
												}else if( linea == scope.alldata[i][j][k].msisdn){ //caso donde la linea esta seteada
													if(contrato == ''){
														//debe retornar solo la linea												
														val = true;
														length = length + 1;
														//obj_aux[j]= aux.splice(linea, 0, scope.alldata[i][j][k]);
														aux.splice(linea, 0, scope.alldata[i][j][k]);
														a[j] = aux;										
			...
			if (length > 0  &&  val == true){
				...
				var myvar = Drupal.t("No se permite la combinación de los filtros Línea y Número de Contrato. Por favor intente de nuevo usando solo uno de los filtros.");
				a[myvar] = {};
				scope.auxScroll = a;
			}
	...
	function CurrentInvoiceController($scope, $http){
		...
		$scope.scroll = function (size) {
			...
			var group;
      		$scope.invoicesByContract.forEach(function(element, index) {  
		          ...
		          if (element.hasOwnProperty(i)) {
		            if (quantity > 0) {
		              if (element[i].length > 0) {
		                var slice = quantity;
		                if (quantity > element[i].length) {
		                  slice = element[i].length;
		                }
		                aux.splice(i, 0, element[i].slice(0, slice));
		                scroll[i] = aux[0];
		                quantity = quantity - element[i].length;
		                if (quantity < 0) {
		                  scroll[i] = aux[0];
		                }
		              }
