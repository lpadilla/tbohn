...
function CompaniesTigoAdminController($scope, $http, el) {
	...
	jQuery('.click-filter-reset').click(function () {
		...
		//reset filters        
		jQuery('.Empresa label').addClass('active');		
		jQuery('.Admin label').addClass('active');
	...
	$scope.statesChanged = function () {
		if (jQuery('#seleccionar-todos-check').prop('checked')) {
			$scope.tigos = {};
			$scope.selection = [];
			$scope.tigos.length = 0;
			$scope.selection.length = 0;
			jQuery('#seleccionar-todos-check').prop('checked', false);
			...
			if (jQuery('.reasign').hasClass('disabled')) {
				jQuery('.reasign').removeClass('disabled');
			}

			jQuery('#seleccionar-todos-check').prop('checked', 'checked');
			if (jQuery('#seleccionar-todos-check').prop('checked', 'checked')) {
				jQuery("input:checkbox").prop('checked', 'checked');
			}

			jQuery("input:checkbox:checked").each(function () {
				$scope.toggleSelection(jQuery(this).val());
			});
