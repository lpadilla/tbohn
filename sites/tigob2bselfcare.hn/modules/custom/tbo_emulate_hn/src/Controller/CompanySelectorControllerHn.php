...
namespace Drupal\tbo_emulate_hn\Controller;
...
/**
 * Class CompanySelectorControllerHn.
 *
 * @package Drupal\tbo_emulate_hn\Controller
 */
class CompanySelectorControllerHn extends ControllerBase {
   ...
   *   Return Hello string.
   */
  public function companySelectorHn($uid) {
    ...
    if ($uid == 0) {
      ...
      if (count($companies) == 1) {
        ...
        foreach ($companies as $data) {
          ...
          $company ['segment'] = $data->segment; //RVS 20180123
  				$company ['company_code'] = $data->company_code; //RVS 20180123
          ...
          } else { //RVS 20180119 parche para evitar notices
				  	$company['environment'] = 'movil';
				  }
        }
      ...
      $config = [];
      $plugin_block = $block_manager->createInstance('company_selector_block_hn', $config);
      ...
      $query->addField('company', 'company_code');  //RVS 20180123
      ...
      $company['segment'] = $companies[0]->segment;
  	  ...
  	  $company ['company_code'] = $companies[0]->company_code; //RVS 20180123
  ...
  public function getCompaniesUser() {
    ...
    $query->addField('company', 'fixed');
    $query->addField('company', 'mobile');
    $query->addField('company', 'segment'); //RVS 20180123
  	$query->addField('company', 'company_code');  //RVS 20180123
