...
/**
 * Class EmulateSessionHn.
 *
 * @package Drupal\tbo_emulate_hn\Controller
 */
class EmulateSessionHn extends ControllerBase {
  
  public function emulateSessionHn($user) {
    \Drupal::service('page_cache_kill_switch')->trigger();
    $service = \Drupal::service('masquerade');

    if ($user != 'unmasquerade') {
      $log = AuditLogEntity::create();
      $segment = 'segmento'; //temporary
      ...
      $name_sup = $account_sup->full_name->value;
     ...
     $_SESSION['ummasquerade_block'] = $plugin_block;
      ...
      return new RedirectResponse(Url::fromUri('internal:/tbo_emulate_hn/selector/0')
        ->toString());
    }
    else {
